...
//init
var bgcolor = "#3299a8"; // mavi
var forecolor = "#a8323c"; // açık kırmızı
var linecolor = "#080707"; //  gri
var linewidth = 3;
var offset = -10;
var yRatio = .4;
var t = 0;
var speed = 0;
var playing = true;
var tus = { ArrowUp:0, ArrowLeft:0, ArrowRight:0 };
...
var player = new function() {
    ...
    this.startBtn.src = "image/basla.png"
    this.leftBtn = new Image();
    this.leftBtn.src = "image/sol.png"
    this.rightBtn = new Image();
    this.rightBtn.src = "image/sag.png"
    this.fireBtn = new Image();
    this.fireBtn.src = "image/gaz.png"

    this.drawInterface = function() {
        if(playing) {
            // interface draw
            ctx.drawImage(this.leftBtn, 20, c.height -100, 80, 80);
            ctx.drawImage(this.rightBtn, 110, c.height -100, 80, 80);
            ctx.drawImage(this.fireBtn, c.width -120, c.height -100, 80, 80);
        } else {
            ...
            ctx.drawImage(this.startBtn, (c.width / 2) - 25, (c.height / 3) + 50,60,60);
        }   
    }
    ...
    this.draw = function() {
        ...
        var p1 = (c.height/2) + noise(this.x + t) * yRatio;
        var p2 = (c.height/2) + noise(this.x + t - 5) * yRatio;
        // gnd check
        var gnd = 0;
        if(p1 - 47 > this.y) {
            this.ySpeed += 0,1;
        }else  {
            this.ySpeed -= this.y - (p1 - 48);
            this.y = p1 - 48;
            gnd = 1;
        }
        ...
        ctx.rotate(this.rot);
        ctx.drawImage(this.truck, -200, -200, 400, 400);
        ctx.restore();
    }
}
...
function draw(){
    ...
    for(let i = offset; i < c.width - offset; ++i){
        ctx.lineTo(i,(c.height/2) + noise(i + t) * yRatio);
    }
...
if(mobilmi) {
    ...
    // desktop controls
    onkeydown = d => tus[d.key] = 1;
    onkeyup = d => tus[d.key] = 0;
